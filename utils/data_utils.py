...
def CER_get_data_case(case_name, exo_variable=[], win=1024, ratio_resample=0.8, seed=0, 
    ...
    if not input_path:
        ...
        if exo_variable:
            extra = pd.read_csv(f"{path_data}ExogeneData/extra_25728.csv")
            extra['date'] = pd.to_datetime(extra['date'])

        data = Add_Exogene_CER(data, df_extra=extra, list_variable=exo_variable, reshape2D=True)        
    else:
        data = pd.read_csv(f"{input_path}/Inputs/{data_name}.csv").set_index('id_pdl')
        case = pd.read_csv(f"{input_path}/Labels/{case_name}.csv").set_index('id_pdl')
        if exo_variable:
            extra = pd.read_csv(f"{input_path}/ExogeneData/{data_name}.csv")
            extra['date'] = pd.to_datetime(extra['date'])
...
def get_data_inference(input_path, house_name, seed=0, win=1024, exo_variable=[]):
    ...
    if exo_variable:
        ...
        # Uncomment this line for POWERCHAINGER house data    
        extra = pd.read_csv(f"{input_path}/ExogeneData/{house_name}.csv")
        extra['date'] = pd.to_datetime(extra['date'])
        data = Add_Exogene_CER(data, df_extra=extra, list_variable=exo_variable, reshape2D=True)
    m = 1 + len(exo_variable)
    slicer = MTWindowSlicer(seed=seed)
    X_data = slicer.fit_transform(data.values, m=m, win=win)
    X_data = np.reshape(X_data, (X_data.shape[0], m, X_data.shape[-1] // m))
    return X_data
