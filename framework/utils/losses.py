...
class GeometricMask(torch.nn.Module):
    ...
    def forward(self, x):
        # Generate a geometric submask for the input tensor x.
        submask = np.ones(x.shape[-1], dtype=bool)
        # Probability of flipping out of a masked segment (avg masked segment = 1/p_m = 24) 
        p_m = 1 / self.mean_length 
        # Probability of flipping out of an unmasked segment (also 24)
        p_u = p_m * self.masking_ratio / (1 - self.masking_ratio) 
        p = [p_m, p_u]
        
        # Randomly choose the initial state (masked or unmasked)
        state = int(np.random.rand() > self.masking_ratio)
        # For each time step in the sequence, decide whether to flip the state
        for i in range(x.shape[-1]):
            # Set the current state in the submask
            submask[i] = state
            # Flip the state with probability p[state]
            if np.random.rand() < p[state]:
                state = 1 - state
        # Create a mask for the training batch
        mask = np.ones(x.shape)
        # Apply submask to the 0th dimension (timeseries)
        # All windows in the batch will have the same samples masked
        mask[:, self.dim_masked, :] = submask
        # Convert the mask to a boolean tensor
        # If the value is True it becomes False which informs not to mask that sample
        # If the value is False it becomes True which informs to mask that sample
        mask = (~torch.Tensor(mask).bool())
        
        # We select 'zero' so the masked is just applied to the batches by torch.mul()
