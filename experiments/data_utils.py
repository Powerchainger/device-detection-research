...
path_data = str(Path(os.getcwd()).resolve().parents[0])
path_data = str(Path(os.getcwd()).resolve()) + '/data/'
...
def CER_get_data_pretraining(seed=0, win=1024, exo_variable=[], entire_curve_normalization=True):
    data = pd.read_csv(path_data+'Inputs/x_residential_25728.csv').set_index('id_pdl')[:100]
